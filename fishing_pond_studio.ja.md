...
![メイン画面の概要](file:///workspace/.images/01_overview.png)
...
![アイテム編集パネル](file:///workspace/.images/02_item_editor.png)

## 3. Minecraft への反映 (デプロイ)

画面下部の「Minecraftへ反映」セクションから、作成したデータを実際のワールドへデプロイできます。

![反映セクション](file:///workspace/.images/03_deploy_section.png)
...
![状態のロード](file:///workspace/.images/04_load_state.png)

これをクリックすると、リソースパック内に保存されているメタデータを読み込み、前回の編集内容（アイテム名や描いたドット絵など）をアプリ上に復元できます。

## 5. バージョン互換性について

v1.1.0 以降、Minecraft 1.21.11 などの最新バージョンでも「非対応」と表示されないよう「三重互換性定義」が採用されています。古い 1.21 系から最新版まで、警告なしで快適に使用いただけます。
